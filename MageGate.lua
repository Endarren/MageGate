...
local portalsID = {}
	...
	portalsID[1259194] = L["Portal_Silvermoon_City"]
...
function MageGate:ChevronNLocked(spell)
	...
	SendChatMessage(string.format(L["CHEVRON_N_ENGAGED"], nTicCount), self:GateGroup())
...
function MageGate:OnInitialize()
	...
	self:SecureHook("ConfirmSummon","SummSound")
...
HBD:GetZoneDistance(oZone, oLevel, oX, oY, dZone, dLevel, dX, dY)
	...
	if isAPortal(spellID)  then
		...
		if MageGate.db.profile.SinglePortalSetting.Active then
			if MageGate.db.profile.SinglePortalSetting.JustYou then
				...
				SendChatMessage(string.format(L["CHEVRON_N_LOCKED"], MageGate.db.profile.SinglePortalSetting.ChevMax), self:GateGroup());
				end
			end
				end
			else
			...
			if unitID == "player" and  MageGate.db.profile.Portal[spellID].AnnounceLock then
				SendChatMessage(string.format(L["CHEVRON_N_LOCKED"], MageGate.db.profile.Portal[spellID].ChevronCount), self:GateGroup());
			end
		end
...
function MageGate:GateGroup ()
	...
	chan = select(2, IsInInstance()) == "pvp" and "BATTLEGROUND" or ((UnitInRaid("player") and "RAID") or (UnitInParty("player")  and "PARTY") or "SAY")
